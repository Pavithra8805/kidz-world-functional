...
function updatePrice() {
    ...
    for (let i = 0; i < products.length; i++) {
        if (products[i].quantity > 0) {
            totalBill += products[i].quantity * (products[i].dollars + products[i].cents);
        }
    }
...
cartBtn.onclick = () => {
    ...
    message += `The total amount is ${finalDollar} dollars and ${finalCents} cents`;
    console.log(`The total amount is ${finalDollar} dollars and ${finalCents} cents`);
    window.open(`https://wa.me/8919480738?text=${message}`);
}